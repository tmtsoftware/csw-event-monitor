...
export const MainWindow = (): JSX.Element => {
  ...
  const numCols = expandedParamInfoModel
    ? 1
    : paramCount == 0
    ? 0
    : Math.round(paramCount / numRows)
  const colIndexes = [...Array.from(Array(numRows * numCols).keys())]

  const cols = colIndexes.map((paramIndex) => {
    const paramInfoModel = expandedParamInfoModel
      ? expandedParamInfoModel
      : paramInfoModels.length > paramIndex
      ? paramInfoModels[paramIndex]
      : undefined
    const cswParamKey = paramInfoModel
      ? ParameterUtil.getCswKey(paramInfoModel)
      : undefined
    const eventInfoModel = paramInfoModel?.eventInfoModel
    const eventKey = eventInfoModel
      ? EventUtil.getEventKey(eventInfoModel)
      : undefined
          ...
          <div
            ...
            {subscriptions.length == 0 ? (
              <div style={{ margin: '100px' }}>
                <Title level={3}>
                  Select events to monitor from the tree at right{' '}
                  <ArrowRightOutlined />
                </Title>
              </div>
            ) : (
